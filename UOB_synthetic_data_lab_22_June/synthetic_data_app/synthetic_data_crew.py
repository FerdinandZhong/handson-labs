...
def _build_tasks(agents: list, inputs: dict) -> list:
    ...
    t_code = Task(
        description=(
            ...
            "CRITICAL — CML Job env var names (use EXACTLY these, no abbreviations):\n"
            "  MANIFEST_PATH  → --manifest  (NOT 'MANIFEST', NOT 'MANIFEST_FILE')\n"
            "  OUTPUT_DIR     → --output    (NOT 'OUTPUT', NOT 'OUTPUT_PATH')\n"
            "  SYNTHETIC_DIR  → --synthetic (NOT 'SYNTHETIC', NOT 'SYNTHETIC_PATH')\n"
            "  REPORT_PATH    → --report    (NOT 'REPORT', NOT 'REPORT_FILE')\n"
            "  ROWS           → --rows\n"
            "  SEED           → --seed\n"
            "Use os.getenv() with these EXACT names as argparse defaults:\n"
            "  import os\n"
